...
function renderOpen(){
  ...
  el.innerHTML = open.map(l=>{
    ...
    return `<div class="open-card rv">
      ...
      <h3>${esc(l.title)} <span style="color:var(--accent)">${esc(l.edition)}</span></h3>
      ...
      <div class="oc-pitch">
        ...
        ${p&&p.head?`<div class="oc-head">${p.head}</div>`:""}
...
function renderList(){
  ...
  c.innerHTML = list.map(l=>{
    ...
    return `<button class="lc rv b-${l.status}" data-open="${l.id}">
      ...
      <h3>${esc(l.title)}${l.edition?` <span style="color:var(--accent)">${esc(l.edition)}</span>`:""}</h3>
